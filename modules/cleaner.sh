...
run_environment_setup() {
    ...
    for pkg in $installed_list; do
        ...
        if $rem_cmd "$pkg" >> "$log_file" 2>&1; then
            # استفاده از ترکیب رنگ مستقیم بدون تداخل با ترمینال‌های قدیمی
            printf "\r\033[K   \033[1;32m✔ Successfully Removed: $pkg\033[0m\n"
        else
            printf "\r\033[K   \033[1;31m❌ Failed to Remove: $pkg (Check log)\033[0m\n"
        fi
        sleep 1 # اصلاح شده: BusyBox فقط اعداد صحیح را برای تاخیر می‌پذیرد
    done
